...
class GSCWOO_googlesheet
{
	...
	public static function preauth($access_code)
	{
		...
		if (is_multisite()) {
			// Fetch API creds
			$api_creds = get_site_option('woogsc_api_free_creds');
		} else {
			// Fetch API creds
			$api_creds = get_option('woogsc_api_free_creds');
		}
	...
	public function auth()
	{
		...
		try {
			...
			wc_gsheetconnector_utility::gs_debug_log("Auth, Error fetching OAuth2 access token, message:" . $e->getMessage());
